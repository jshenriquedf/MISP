...
# Log WORNING
function logworning () { log "${COLOR_BROWN}[FALHA]${COLOR_NC}" "${COLOR_GREEN}${1}${COLOR_NC}" "${2}" ; }

# Log ERROR
function logerror () { log "${COLOR_RED}[ERROR]${COLOR_NC}" "${COLOR_GREEN}${1}${COLOR_NC}" "${2}" ; }
###################### FIM FUNÇÕES DE LOG -----------------------------------------------
...
function localSwap () {
  if [[ ${SWAP_ENABLE} == true ]]; then
    if [[ ! $(sudo swapon -v) ]]; then
      ...
      [[ ! $(sudo chattr +C ${SWAP_FILE} 2> /dev/null) ]] && logworning "Falha na execução do comando: chattr +C"
...
function configRepoMISP (){
  ...
  if [[ ! -d ${PATH_TO_MISP} ]]; then
    ...
    ${SUDO_WWW} git submodule sync --recursive
...
function configRepoMISPDep (){
  ...
  if [[ -d ${PATH_TO_MISP} ]]; then
    ...
    #sudo zypper in -y libcaca-devel cmake liblua5_3-5
    #cd /tmp
    #false; while [[ $? -ne 0 ]]; do [[ ! -d "faup" ]] && sudo git clone https://github.com/stricaud/faup.git faup; done
    #false; while [[ $? -ne 0 ]]; do [[ ! -d "gtcaca" ]] && sudo git clone https://github.com/stricaud/gtcaca.git gtcaca; done
...
function configProxyMISP () {
  if [[ ${ADMIN_PROXY} == true ]]; then
    ...
    sudo cp ${MISP_CA_DEFAULT} ${MISP_CA_DEFAULT}.old
...
function configuracaoMISP (){
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.ca_path "${MISP_CA_DEFAULT}"
...
function checkUsrLocalSrc () {
  ...
  [[ ! -e /usr/local/src ]] && mkdir -p /usr/local/src
