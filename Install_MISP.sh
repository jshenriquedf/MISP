...
#php -r "xdebug_info();"
  if [[ ${SUPERVISOR_ENABLE} == true  ]]; then
    ...
    if [[ ! -z ${ADMIN_PROXY} ]]; then
      sudo pip3 install --proxy=${ADMIN_PROXY} -U supervisor
    else
...
function mispmodulesRHEL () {

  eval $(sudo grep -v -E "^\s*(;|$|\[|#)" /admin/.env | xargs -i echo {} | sed 's/=/=\"/;s/$/\"/')

  sudo zypper in -y gcc-c++ openjpeg-devel libpoppler-devel python3-pkgconfig

  sudo chmod 2777 /usr/local/src
  sudo chown ${WWW_USER}:${WWW_USER} /usr/local/src
  cd /usr/local/src/
  ...
  # pip install
  #${SUDO_WWW} ${PATH_TO_MISP}/venv/bin/pip install pytz==2022.7.1
  #${SUDO_WWW} ${PATH_TO_MISP}/venv/bin/pip install pymisp==2.4.185
...
function configuracaoMISP (){
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.ca_path ${ADMIN_CERT_DEST_PER}
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.email "${ADMIN_EMAIL}"
  # Envio de e-mail
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.redis_host "${REDIS_HOST}"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.redis_port ${REDIS_PORT}
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.redis_database ${REDIS_DATA}
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting MISP.redis_password "${REDIS_PASS}"
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.redis_host  "${REDIS_HOST}"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.redis_port ${REDIS_PORT}
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.redis_database 1
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.redis_password ${REDIS_PASS}
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.supervisor_host "${SUPERVISOR_HOST}"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.supervisor_port ${SUPERVISOR_PORT}
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.supervisor_user "${SUPERVISOR_USER}"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting SimpleBackgroundJobs.supervisor_password "${SUPERVISOR_PASS}"
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting Plugin.ZeroMQ_redis_host "${REDIS_HOST}"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting Plugin.ZeroMQ_redis_port ${REDIS_PORT}
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting Plugin.ZeroMQ_redis_password "${REDIS_PASS}"
  ...
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting --force MISP.footermidleft "Powered by MISP"
  ${SUDO_WWW} ${RUN_PHP} -- ${CAKE} Admin setSetting --force MISP.footermidright "Powered by MISP"
